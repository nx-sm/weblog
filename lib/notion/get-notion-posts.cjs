	...
	for (const post of posts) {
		n2m
			...
			.then((mdblocks) => {
				...
				if (fs.existsSync(path.join(process.cwd(), "src", "text.md"))) {
					console.log(
						"Test File already exist: " + fs.existsSync(path.join(process.cwd(), "src", "text.md"))
					);
				} else {
					fs.writeFileSync(fs.existsSync(path.join(process.cwd(), "src", "text.md"))),
						"test",
						{ mode: 0o644 };
					console.log(
						"File written successfully! " +
							fs.existsSync(path.join(process.cwd(), "src", "text.md"))
					);
				}
